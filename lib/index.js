...
// you have an Array<Either Error String> and you want to get an Array<String> but only if all Eithers are Right. If at least one Left exists

// instead of `Array (Either Error String)` you should have an `Either Error (Array String)`

// validateParts :: [Array<Either Error String>, Array<Either Error String>, Array<Either Error String>] -> Either Array<Error> [ids, allDependencies, allDependents]
// - - -
...
// TODO: Add type annotation comment
function extractPartsFactory (idMapper = mappers.id, dependenciesMapper = mappers.dependencies, dependentsMapper = mappers.dependents) {
  ...
  return input => {
    ...
    const combinedAsEither = compose(Either.fromValidation, combineValidations)
    return [ combinedAsEither(ids), combinedAsEither(allDependencies), combinedAsEither(allDependents) ]
  }
}
